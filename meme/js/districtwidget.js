$.fn.districtselector = function(options) {
    ...
    var defaults = {
        ...
        stockdata:"",
        showproposal:true,
        selecorFuntion: function(id, name, option) {
            var $this = $(this);
    ...
    function createreference(category_arr) {
        ...
        for (i; i < len; i++) {
            reference_dom += "<div class='province-c'>" +
                "<div class='province-t'>" + reference_arr[i] + "</div>" +
                "<div class='province-l' data-reference='" + reference_arr[i] + "'></div>" +
                "</div>";
        }
    ...
    function searchdistrict(data,value){
        var data_arr = data.msg,
            len = data_arr.length,
            dom = "<div class='search-title'>"+ value +" 若需缩小范围，请输入更多信息.</div>"+
    ...
    function keyAction(){
        ...
        document.onkeyup=function(e){
            ...
            if(e.keyCode==38){
                ...
                key_index-=1;
                key_index=key_index%sLength;
                if(key_index<0){key_index+=sLength};
                searchItem.eq(key_index).addClass('hover').siblings().removeClass('hover');
            };
            // enter键选择
            if(e.keyCode==13){
    ...
    return this.each(function() {
        ...
        if(defaults.showproposal){
            dom = "<div class='mod-district-box' data-id=" + ID + ">" +
                "<div class='district-wrap'>"+
                "<div class='district-tips'>支持中文，拼音搜索城市</div>" +
                "<ul class='district-nav'></ul>" +
                "<div class='district-cont'>" +
                "<ul class='district-sidebar'></ul>" +
                "<div class='district-tabcont'></div>" +
                "<div class='district-tabcont' style='display:none'></div>" +
                "</div></div><div class='search-wrap'></div>" +
                "</div>";
        }else{
            dom = "<div class='mod-district-box' data-id=" + ID + ">" +
                "<div class='district-wrap'>"+
                "<div class='district-tips'>支持中文，拼音搜索城市</div>" +
                "</div><div class='search-wrap'></div>" +
                "</div>";
        };
